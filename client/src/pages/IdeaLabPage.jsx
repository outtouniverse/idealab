...
function IdeaLabPage() {
  ...
  const handleIdeaSubmit = async (ideaData) => {
    ...
    try {
      const dbRes = await fetch("https://idealab-beta2.vercel.app/idealab/new", {
        method: "POST",
    ...
    try {
      ...
      if (newIdeaLabId) {
        const saveRes = await fetch(`https://idealab-beta2.vercel.app/idealab/save-output/${newIdeaLabId}`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
